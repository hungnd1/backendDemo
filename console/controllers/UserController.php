...
class UserController extends Controller
{
     ...
     */
    public function actionCreateSubscriberService($service_id,$subscriber_id,$service_provider_id){
        $sc = new SubscriberServiceAsm();
        $sc->service_provider_id =$service_provider_id;
        if($service_provider_id ==1){
            $sc->service_id =$service_id;
            $sc->service_name = "Pk1";
        }elseif($service_provider_id ==2){
            $sc->service_id =$service_id;
            $sc->service_name = "Xp2";
        }
        $sc->subscriber_id =3;
    ...
    /**
     * Sample: ./yii be-user/create-cp-user "huydq" "huydq@vivas.vn" "123456" 1
     * @param $username
     * @param $email
     * @param $password
     * @param $sp_id
     * @throws Exception
     */
    public function actionCreateCpUser($user, $email, $password,$sp_id) {
        $sp_user = $this->actionCreateUser($user, $email, $password);
        $sp_user->service_provider_id = $sp_id;
        $sp_user->content_provider_id = $sp_id;
        $sp_user->type = User::USER_TYPE_CP;
        $sp_user->update();
    }
